...
function Home() {
  ...
  return (
    ...
    <div className="container">
        <h2 className="mt-3"> AXIOS TEST </h2>
      {/* 반복문 사용법 : dept.map(()=>{return (<li>{value.dname}</li>)}) */}
      <ul className="list-group">
        {
            dept.map((value, index)=> (<li className="list-group-item" key={index}>{value.dname}</li>))    
        }
      </ul>
    </div>
  );
